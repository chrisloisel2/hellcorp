...
GUIDES_DIR = "/Users/christopher/HellCEO/hellcorp/sdxl_lora_bench/out/lucy_walk_ebsynth_full/guides"

OUT_DIR = "/Users/christopher/HellCEO/hellcorp/sdxl_lora_bench/out/lucy_walk_ebsynth_full"
...
N = len(frames)
KEYFRAME_IDX = list(range(0, N, 2))
if KEYFRAME_IDX[-1] != N - 1:
    KEYFRAME_IDX.append(N - 1)
print(f"{N} frames, keyframes ({len(KEYFRAME_IDX)}) = {KEYFRAME_IDX}", flush=True)
...
for i in KEYFRAME_IDX:
    ...
    if os.path.exists(dst):
        print(f"[keyframe {i}] already done, skipping", flush=True)
        continue
...
for a, b in zip(KEYFRAME_IDX[:-1], KEYFRAME_IDX[1:]):
    ...
    for i in range(a + 1, mid + 1):
        ...
        dst = os.path.join(OUT_DIR, cur_name)
        if os.path.exists(dst):
            prev_name = cur_name
            continue
        t0 = time.time()
        ebsynth_step(os.path.join(OUT_DIR, prev_name), prev_name, cur_name, dst)
...
print("FULL_EBSYNTH_DONE", flush=True)
