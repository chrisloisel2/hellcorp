...
RUN_NAME = "walking_bitch_summer_memories_chained"
...
# Step 1: guides = raw deterministic 3D composite for every frame
for path in frames:
...
# Step 2: stylize each keyframe independently (verified consistent design at these settings)
device = "mps"
...
for i in KEYFRAME_IDX:
    ...
    out.save(os.path.join(KEYFRAME_DIR, name))
...
# Every output frame is always derived from its immediate neighbour (fixes texture "boiling"),
# and no chain runs longer than ~half the keyframe spacing (bounds blur/drift).
for a, b in zip(KEYFRAME_IDX[:-1], KEYFRAME_IDX[1:]):
    mid = (a + b) // 2
    # forward chain: a -> a+1 -> ... -> mid
